	...
	try {
		if (!clientId || !clientSecret) {
			return res
				.status(500)
				.send(
					'Missing LXC_CLIENT_ID or LXC_CLIENT_SECRET environment variables. Ensure these are set in your environment.'
				);
		}
		...
		const apiResponse = await axios.get(
			`${liferayUrl}/o/headless-delivery/v1.0/sites`,
			{
				headers: {
